...
def get_model_master(params):
    ...
    if params['block_type'] == 'dense':
        block = dense_block
    elif params['block_type'] == 'GRU':
        block = GRU_block
    elif params['block_type'] == 'LSTM':
        block = LSTM_block
    elif params['block_type'] == 'conv1D':
        block = conv1D_block
    try:
        conv_dp = params['conv_dp']
    except KeyError:
        conv_dp = False
...
def get_task_block(model_name):
    ...
    if model_name == RELATIVE_ANGLE \
        ...
        while True:
            dir_layers.append(l)
            try:
                l = get_prev_layer(model, l, list_idx=1)
            except:
                break  # reached dir_input layer
