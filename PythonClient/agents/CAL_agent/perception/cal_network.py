...
[RED_LIGHT, HAZARD_STOP, SPEED_SIGN, RELATIVE_ANGLE, CENTER_DISTANCE,
 VEH_DISTANCE] = last_layer_keys
...
class TaskBlockEnsemble(object):
    ...
    def predict(self, im, meta):
        ...
        preds2 = self.model2.predict([im[:, (14 - self.inp_shape2):, :, :, :]] + meta, batch_size=1,
                                     verbose=0)
        preds3 = self.model3.predict([im[:, (14 - self.inp_shape3):, :, :, :]] + meta, batch_size=1,
                                     verbose=0)
        preds4 = self.model4.predict([im[:, (14 - self.inp_shape4):, :, :, :]] + meta, batch_size=1,
                                     verbose=0)
