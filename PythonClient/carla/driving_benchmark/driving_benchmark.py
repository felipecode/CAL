...
class DrivingBenchmark(object):
    ...
    def _run_navigation_episode(
        ...
        ### own metrics
        stuck_vec = [0] * 20 # measure for 60 frames
        center_distance_vec = []
        ...
        while not fail and not success:
            ...
            collision_other -= old_collision_value
            otherlane = measurements.player_measurements.intersection_otherlane
            offroad = measurements.player_measurements.intersection_offroad
            # if still driving or got unstuck (v > 5km/h)
            if measurements.player_measurements.forward_speed*3.6 > 4:
                cycle_signal(stuck_vec, 0)
                old_collision_value = collision_other
            elif offroad or otherlane or collision_other:
                cycle_signal(stuck_vec, 1)

            if all(stuck_vec): break
            ###
